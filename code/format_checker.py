...
def check_format(line):
    ...
    for key in replace_dict:
        ...
        if len(rp) > 0:
            wrong_idx_list += rp
            print 'REPLACE:' + key.encode('utf-8')
    ...
    for i in range(2, len(line)):
        if re.match(chinese_word_pattern, line[i]) and re.match(chinese_word_pattern, line[i-2]) and line[i-1] == ' ':
            print 'CHINESE WORD SPACE:' + line[i-2].encode('utf-8') + ' '  + line[i].encode('utf-8')
            del_idx.append(i-1)
    ...
    for i in range(1, len(line) - 1):
        if line[i] in notes:
            if line[i-1] == ' ' or line[i+1] == ' ':
                print 'FULL NOTE SPACE:' + line.encode('utf-8')
    ...
    if len(wrong_bracket) > 0:
        ...
        print 'INVALID BRACKET:' + word.encode('utf-8')
...
#            continue
        ...
        if s in eng_dict:
            if s != eng_dict[s]:
                print 'ENGLISH TERMS ERROR:' + s + ' to ' + eng_dict[s]
